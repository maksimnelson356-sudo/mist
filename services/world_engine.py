...
class WorldEngine:
    ...
    async def _on_new_day(self, old_day: int, new_day: int):
        ...
        if events_today:
            await self.chronicle.publish(
                ...
                f"День {new_day} — мир не стоит на месте. События происходят.",
    ...
    ALLOWED_LOCATION_FIELDS = frozenset({
        "danger_level", "food_supply", "tree_density", "magic_level",
        "creature_count", "population", "wealth", "reputation",
    })
    ...
    async def _apply_event_effects(self, location_id: str, effects: dict, db=None):
        ...
        safe_effects = {f: d for f, d in effects.items() if f in self.ALLOWED_LOCATION_FIELDS}
        if not safe_effects:
            return

        if db:
            for field, delta in safe_effects.items():
                await db.execute(
