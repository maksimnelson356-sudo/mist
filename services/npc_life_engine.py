...
class NPCLifeEngine:
                ...
                for j in range(i + 1, len(npc_list)):
                    ...
                    if rel:
                        ...
                        await db.execute(
                            ...
                            .values(value=new_value, events=events, last_interaction_at=datetime.utcnow())
                        )
                    else:
                        new_rel = NPCRelationshipModel(
                            ...
                            value=delta,
                            events=[{"day": datetime.utcnow().isoformat(), "delta": delta, "reason": "первая встреча"}],
                        )
                        db.add(new_rel)
    ...
    async def _process_lifecycle(self, game_day: int, season: str):
        async for db in get_db():
            ...
            if random.random() < 0.03 and len(npcs) < 15:
